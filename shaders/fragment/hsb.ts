...
const hsbSomething = tgpu["~unstable"].fragmentFn({
  ...
  const baseAngle = std.atan2(toCenter.y, toCenter.x) + (time % (2 * Math.PI));
  const radius = std.length(toCenter) * 2.0;
  // const angle =
  //   utils.smoothstep(-Math.PI, Math.PI, baseAngle) * 2 * Math.PI - Math.PI;
  const angle = std.select(
    baseAngle,
    utils.smoothstep(-Math.PI, Math.PI, baseAngle) * 2 * Math.PI - Math.PI,
    baseAngle > Math.PI / 6 && baseAngle < -Math.PI / 6
  ); //std.tanh(0.5 * baseAngle) * Math.PI * 1.1;
